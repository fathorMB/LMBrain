...
pub fn get_github_pat() -> Result<Option<String>, String> {
    match github_pat_entry()?.get_password() {
        ...
        Err(error) => Err(format!(
            "Could not read the GitHub PAT from the credential store: {error}"
        )),
    }
}
...
pub fn save_github_pat(token: &str) -> Result<(), String> {
    ...
    github_pat_entry()?.set_password(token).map_err(|error| {
        format!("Could not save the GitHub PAT in the credential store: {error}")
    })?;
    ...
    if get_github_pat()?.as_deref() != Some(token) {
        return Err(
            "The credential store accepted the GitHub PAT but did not persist it.".to_string(),
        );
    }
...
pub fn delete_github_pat() -> Result<(), String> {
    match github_pat_entry()?.delete_credential() {
        ...
        Err(error) => Err(format!(
            "Could not delete the GitHub PAT from the credential store: {error}"
        )),
    }
}
...
fn fetch_pull_requests(
    ...
    let response = match req.call() {
        ...
        Err(ureq::Error::Status(status, res)) => {
            ...
            return Err(format!(
                "GitHub API Pulls request failed with status {status}: {body}"
            ));
        }
        Err(e) => {
    ...
    let json_array: serde_json::Value = response
        .into_json()
...
fn fetch_workflow_runs(
    ...
    let response = match req.call() {
        ...
        Err(ureq::Error::Status(status, res)) => {
            let body = res.into_string().unwrap_or_default();
            return Err(format!(
                "GitHub API Runs request failed with status {status}: {body}"
            ));
        }
        Err(e) => {
    ...
    if let Some(runs_arr) = json_response
        .get("workflow_runs")
...
mod tests {
    ...
    fn native_windows_credential_store_persists_across_entries() {
        ...
        let second_entry =
            keyring::Entry::new(&service, username).expect("re-open credential entry");
