...
describe("RepositoryView", () => {
  ...
  it("does not render or describe a branch graph", async () => {
    render(<RepositoryView />);

    await waitFor(() => expect(screen.getAllByText(/#12 Fix sidebar/i).length).toBeGreaterThan(0));
    expect(screen.queryByText(/branch graph/i)).toBeNull();
    expect(screen.queryByLabelText(/branch commit graph/i)).toBeNull();
  });
