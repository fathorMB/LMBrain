 ...
 * assumption:
 * - delegate: let xterm handle the event natively (normal-buffer scrollback,
 *   mouse reports when the TUI tracks the mouse, or alternate-screen arrow
 *   emulation).
...
export function terminalWheelAction(
  ...
  if (bufferType === "normal") {
    ...
    // normal scrollback. Preserve xterm's native wheel event path here.
