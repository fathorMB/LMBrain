...
7. Propose a plan to resolve any version parsing or compatibility issues, keeping the operator informed of any risks.
8. Do not blindly overwrite any customized files or project-specific content.
9. Only update the \`VERSION\` file in \`.lmbrain/VERSION\` once the version issues have been resolved and validation checks succeed.`;
  }
...
8. Do not blindly overwrite any customized files or project-specific content.
9. Request operator confirmation before performing any changes.
10. Update the version identifier in \`.lmbrain/VERSION\` only after all manual migration steps and validation succeed.`;
  }
...
7. Produce a detailed migration plan, preserving any project-specific customizations and existing active/inactive agent statuses.
8. Do NOT blindly overwrite or discard project-specific edits or customized kit files.
9. Present the migration plan and ask for operator confirmation before performing any additive migration writes.
10. If any required changes are breaking or ambiguous, create a migration spec or report the conflicts clearly to the operator.
11. Update the version identifier in \`.lmbrain/VERSION\` to "${bundledVersion}" only after all migration steps and validation succeed, and the operator approves.`;
}
