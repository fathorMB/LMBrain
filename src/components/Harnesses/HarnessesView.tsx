  ...
  return (
    <PageShell archetype="dense">
      <PageHeader
        ...
        description="User-level agent CLIs available to LMBrain. Updates are manual, use each harness's supported self-updater, and never modify project files."
        actions={
      ...
      />
        ...
        <div role="note" style={{ display: "flex", gap: 9, padding: "10px 12px", marginBottom: 16, border: "1px solid rgba(91,141,239,.2)", borderRadius: 8, background: "rgba(91,141,239,.07)", color: "var(--text-secondary)", fontSize: "var(--text-xs)", lineHeight: 1.5 }}>
        ...
        <label style={{ display: "block", padding: 12, marginBottom: 16, border: "1px solid var(--border-secondary)", borderRadius: 8, background: "var(--bg-tertiary)" }}>
          <span style={{ display: "block", marginBottom: 4, fontSize: "var(--text-sm)", fontWeight: 700 }}>Codex executable override</span>
          <span style={{ display: "block", marginBottom: 8, fontSize: "var(--text-xs)", color: "var(--text-tertiary)" }}>Machine-local path used by both probing and new Codex sessions.</span>
  ...
  return (
    <section style={{ display: "flex", flexDirection: "column", minHeight: 310, padding: 16, borderRadius: 10, border: "1px solid var(--border-secondary)", borderTop: `2px solid ${presentation.color}`, background: "var(--bg-tertiary)" }}>
      <div style={{ display: "flex", alignItems: "start", justifyContent: "space-between", gap: 12 }}>
        <div>
          <div style={{ fontSize: "var(--text-lg)", fontWeight: 800, color: "var(--text-primary)" }}>{status.label}</div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 5, fontSize: "var(--text-xs)", color: presentation.color }}>
            <span aria-hidden="true" style={{ width: 7, height: 7, borderRadius: "50%", background: presentation.color }} />
            {presentation.label}
          </div>
        </div>
        {status.version && <span style={{ fontFamily: "var(--font-mono)", fontSize: "var(--text-sm)", color: "#c9bdf9" }}>{status.version}</span>}
      </div>
      ...
      <div style={{ marginTop: 16, flex: 1 }}>
        ...
        {status.detail && <div role={status.state === "error" ? "alert" : undefined} style={{ marginTop: 10, fontSize: "var(--text-xs)", lineHeight: 1.45, color: status.state === "error" ? "#e9857b" : "var(--text-tertiary)" }}>{status.detail}</div>}
        {blocked && (
          <div role="alert" style={{ marginTop: 10, padding: "7px 8px", borderRadius: 6, background: "rgba(224,162,58,.08)", color: "#d9b86d", fontSize: "var(--text-xs)", lineHeight: 1.4 }}>
            Close running session{runningSessions.length === 1 ? "" : "s"}: {runningSessions.join(", ")}
          </div>
        )}
        {error && <div role="alert" style={{ marginTop: 10, color: "#e9857b", fontSize: "var(--text-xs)", lineHeight: 1.45 }}>{error}</div>}
...
function UpdateResultDetails({ result }: { result: HarnessUpdateResult }) {
  ...
  return (
    <details style={{ marginTop: 11, border: "1px solid rgba(255,255,255,.07)", borderRadius: 6, background: "rgba(255,255,255,.025)" }}>
      <summary style={{ cursor: "pointer", padding: "7px 8px", color, fontSize: "var(--text-xs)", fontWeight: 700 }}>{label}</summary>
      <div style={{ padding: "0 8px 8px", fontSize: "var(--text-xs)", color: "var(--text-tertiary)", lineHeight: 1.45 }}>
        <div>{result.before.version ?? "unknown"} → {result.after.version ?? "unknown"}</div>
        {(result.stdout || result.stderr) && (
          <pre style={{ margin: "7px 0 0", maxHeight: 140, overflow: "auto", whiteSpace: "pre-wrap", overflowWrap: "anywhere", color: "var(--text-secondary)", fontFamily: "var(--font-mono)", fontSize: "var(--text-2xs)" }}>
  ...
  return (
    <div role="presentation" style={{ position: "fixed", inset: 0, zIndex: 12000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24, background: "rgba(4,3,6,.72)", backdropFilter: "blur(5px)" }}>
      <div role="dialog" aria-modal="true" aria-labelledby="harness-update-title" style={{ width: "min(500px, 100%)", padding: 20, borderRadius: 12, border: "1px solid #332d3e", background: "#15111b", boxShadow: "0 20px 70px rgba(0,0,0,.5)" }}>
        <h2 id="harness-update-title" style={{ margin: "0 0 9px", fontSize: "var(--text-xl)" }}>Update {status.label}?</h2>
        <p style={{ margin: "0 0 12px", color: "var(--text-secondary)", fontSize: "var(--text-sm)", lineHeight: 1.55 }}>
        ...
        <div style={{ padding: "9px 10px", borderRadius: 7, background: "rgba(224,162,58,.08)", color: "#d9b86d", fontSize: "var(--text-xs)", lineHeight: 1.5 }}>
...
function Detail({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div style={{ marginBottom: 9 }}>
      <div style={{ fontSize: "var(--text-2xs)", textTransform: "uppercase", letterSpacing: ".07em", color: "var(--text-muted)", marginBottom: 3 }}>{label}</div>
      <div title={value} style={{ fontSize: "var(--text-xs)", color: "var(--text-secondary)", fontFamily: mono ? "var(--font-mono)" : undefined, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{value}</div>
    </div>
  );
}
