...
pub fn list_maps(root: impl AsRef<Path>) -> Vec<WayfinderMap> {
    scan(root.as_ref().join(".lmbrain/wayfinder/maps"), parse_map)
}
...
fn mutate_ticket(
    ...
    doc.set("updated", &today());
    doc.set("wayfinder_events", &format!("[{}]", event(status, actor)));
    atomic_write(&path, &doc.render())?;
...
fn validate_map(map: &WayfinderMap, tickets: &[WayfinderTicket]) -> Vec<String> {
    ...
    for ticket in tickets.iter().filter(|ticket| ticket.map == map.id) {
...
fn scan<T>(dir: PathBuf, parse: fn(&Path, &Document) -> T) -> Vec<T> {
    let mut items = vec![];
...
fn next_id(root: &Path, prefix: &str) -> String {
    let values: Vec<String> = if prefix == "MAP" {
        list_maps(root).into_iter().map(|map| map.id).collect()
    } else {
        list_tickets(root)
            .into_iter()
    ...
    let max = values
        .into_iter()
        .filter_map(|id| id.strip_prefix(&format!("{prefix}-"))?.parse::<u32>().ok())
        .max()
        .unwrap_or(0);
