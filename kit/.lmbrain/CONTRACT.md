...
- Verification authority is distinct: `agent`/`kit` requirements belong to `before-submit`; `lead`/`operator` requirements belong to `before-done`. Lead and operator requirements need both an already-checked checklist item and a fresh typed attestation from the matching authority. Attestation records evidence only: it never approves a spec, checks the item, or changes lifecycle status. Lead uses `spec_attest_lead`; the human operator attests through the desktop Operations page, or — when approval was granted out of band, e.g. in conversation with the Lead — the Lead records it with `spec_attest_operator_delegated`, naming the operator, the channel, and the quoted authorization. A delegated attestation satisfies the gate without force and remains auditable as delegated; it records the channel of consent, never the Lead's own judgement. Normal `spec_submit`/`spec_done` report every blocker; forced transitions retain the blocker details in the audit trail and continue to mean the requirement was not met. Legacy completed specs remain completed and surface unresolved gates as diagnostics.
