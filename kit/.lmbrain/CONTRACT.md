...
The manifest is strict: unknown fields, arbitrary commands, scripts, hooks, absolute machine paths, traversal, secret-like environment keys, and unsupported host capabilities are invalid. Machine-local executable selection and credentials never belong in it. Repository intent does not authorize effects: materialization requires a separate machine-local approval bound to the canonical manifest digest and workspace identity.

Controlled MCP access uses `harness_config_get`, `harness_config_validate`, and `harness_config_set`. The setter replaces the complete validated manifest atomically and records digest-only audit evidence; it never grants approval or materializes native harness files by itself. Approval and materialization are distinct Project Lead actions through `harness_manifest_approve`, `harness_config_apply`, and `harness_approval_revoke`: approval binds the exact previewed digest, apply refuses conflicts and non-ready hosts, and every action is audited with its actor. `harness_approval_status`, `harness_plan_preview`, and `harness_drift_status` are the read-only counterparts. The desktop application consults this state read-only on its Environment page and exposes no mutation.
...
The application exposes verification state read-only on its Environment page and must not couple any manifest action to an artifact lifecycle transition. The Project Lead manages the manifest lifecycle through `verification_manifest_init`, `verification_manifest_validate`, `verification_manifest_set`, `verification_manifest_rollback`, and grants approval through the separate digest-bound `verification_manifest_approve`.
...
The Project Lead may call `lmbrain_feedback_record` autonomously. The operation is locked and atomic, appends rather than rewrites history, validates relations and taxonomy, and never changes an artifact status. `lmbrain_feedback_report` is read-only and does not create an absent report. `lmbrain_feedback_resolve` appends a lifecycle event to an existing note without editing it: `resolved` retires the note against a named LMBrain release, `reconfirmed` records that a still-open note reproduces on a later version without minting a new note ID. A note's status (`open`/`resolved`) is derived at read time from these events; a resolved note accepts no further events. Notes must omit credentials, personal data, proprietary source excerpts, and unnecessary project content; evidence uses the minimum reproducible context required by the LMBrain product team.
...
- Verification authority is distinct: `agent`/`kit` requirements belong to `before-submit`; `lead`/`operator` requirements belong to `before-done`. Lead and operator requirements need both an already-checked checklist item and a fresh typed attestation from the matching authority. Attestation records evidence only: it never approves a spec, checks the item, or changes lifecycle status. Lead uses `spec_attest_lead`; the human operator attests through the desktop verification panel, or — when approval was granted out of band, e.g. in conversation with the Lead — the Lead records it with `spec_attest_operator_delegated`, naming the operator, the channel, and the quoted authorization. A delegated attestation satisfies the gate without force and remains auditable as delegated; it records the channel of consent, never the Lead's own judgement. Normal `spec_submit`/`spec_done` report every blocker; forced transitions retain the blocker details in the audit trail and continue to mean the requirement was not met. Legacy completed specs remain completed and surface unresolved gates as diagnostics.
...
- Governed review writes validate agent attribution: an `implementation_agent` or `remediation_agent` that is an unreplaced template placeholder (`AGENT-XXX`) or does not resolve to an existing `AGENT-*` profile is rejected, so it can never poison per-profile effectiveness metrics. A provably wrong attribution on an existing review is corrected with `review_set_implementation_agent`, which fixes the field and appends an `attribution-correction` event recording the previous value — history is never edited. Unresolved attributions on existing artifacts surface as `review-attribution-unresolved` diagnostics.
