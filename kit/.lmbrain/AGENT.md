...
A spec reaches `done` only with its acceptance criteria satisfied or validly waived, evidence recorded, and an accepted review. Drive these transitions with the `lmbrain-mcp` spec verbs (`spec_ready`/`spec_start`/`spec_submit`/`spec_done`/`spec_discard`); keep the `status` frontmatter and the spec's folder in agreement. Hard `depends_on` prerequisites must all be done before normal readiness/start. Change them only with `spec_dependencies_set` while backlog. Use `spec_park` for a reasoned `ready -> backlog`; never simulate parking with manual frontmatter/file moves.

Acceptance criteria use exactly three markers: `- [ ]` not satisfied, `- [x]` satisfied, and `- [~] <criterion> | waived=DEBT-xxx` impeded and waived against an existing debt, with the original criterion text preserved. Do not invent a marker for an impediment: any other marker counts as not satisfied and is reported by name, so an honest declaration written in an unrecognized shape is treated as an omission. When a criterion cannot be satisfied, record the debt first, then waive the criterion against it. Never tick a criterion you cannot evidence.

Declare the executable gates a spec must pass with `spec_set_verification_gates`, or through the `verification_gates` field at `lmbrain_create`. The IDs must exist in the current verification manifest, and the set can only be changed while the spec is in `backlog`, `ready`, or `working`. A spec with no declared gates executes none, however complete the approved manifest is.
