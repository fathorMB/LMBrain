...
- `debt_create`, `debt_plan`, `debt_defer`, `debt_resolve`, `debt_accept_risk`, `debt_supersede`, and `debt_reopen` own the `DEBT-*` lifecycle. Core validates global identity, status directories, typed audit events, source-pair uniqueness, typed references, blocker cycles, authority, and resolution evidence under the shared mutation locks.
- `debt_context` derives bounded forward/reverse relations without rewriting review history; `debt_candidates` inventories canonical `RF-*` review findings read-only and never infers disposition. `debt_migration_preview` and confirmed digest-bound `debt_migrate` provide the only legacy conversion path. Project/spec/review contexts and shared diagnostics consume the same core debt index.
- The desktop Debts route is read-only: filters, sorting, canonical relationship navigation, contextual counts, and governed prompt copying do not expose lifecycle transitions. Review verdicts and agent metrics remain review-based and are not recomputed from promoted debts.
