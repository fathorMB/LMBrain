...
use lmbrain_core::transitions::{create, set_recommended_agent, set_task_evidence, transition, ArtifactKind, CreateRequest, MutationOptions};
...
fn tools()->Vec<Value>{let mut entries=vec![];for(name,description)in[("task_plan","Promote a task to planned."),("task_start","Start a planned task."),("task_submit","Submit an in-progress task for review."),("task_complete","Complete a reviewed task."),("task_block","Block a task."),("task_cancel","Cancel a task."),("spec_ready","Make a proposed spec ready."),("spec_start","Start a ready spec."),("spec_submit","Submit a spec for review."),("spec_accept","Accept a reviewed spec."),("review_accept","Accept a review.")]{entries.push(transition_tool(name,description));}entries.extend([create_tool(),setter_tool("lmbrain_set_recommended_agent","Set a spec recommended agent.","agent"),setter_tool("lmbrain_set_task_evidence","Set task evidence.","evidence"),read_tool("lmbrain_get_artifact","Read a repository artifact."),read_tool("lmbrain_validate","Validate controlled-mutation invariants."),read_tool("lmbrain_list_ready_handoffs","List ready handoffs.")]);entries}
...
fn create_tool()->Value{json!({"name":"lmbrain_create","description":"Create an artifact with an allocated ID.","inputSchema":{"type":"object","required":["kind","title"],"properties":{"kind":{"type":"string","enum":["task","spec","review","adr","agent","agent-proposal","mcp","mcp-proposal","handoff"]},"title":{"type":"string"},"status":{"type":"string"},"fields":{"type":"array","items":{"type":"array","items":{"type":"string"},"minItems":2,"maxItems":2}}},"additionalProperties":false}})}
...
mod tests {
    ...
    #[test]
    fn adr_accept_is_not_agent_callable() {
        // Accepting ADRs is the operator's prerogative; it must not be an agent tool.
        ...
        let names: Vec<String> = super::tools()
            ...
            .collect();
        assert!(!names.contains(&"adr_accept".to_string()), "adr_accept must not be exposed to agents");
        // The lead-permitted acceptances stay available.
        assert!(names.contains(&"spec_accept".to_string()));
