use std::{io::{BufRead, BufReader, Write}, process::{Command, Stdio}, fs};
use tempfile::tempdir;
#[test] fn protocol_drives_spec_transition() {
    let d=tempdir().unwrap();
    fs::create_dir_all(d.path().join(".lmbrain/specs/ready")).unwrap();
    fs::write(d.path().join(".lmbrain/specs/ready/SPEC-001.md"),"---\nid: SPEC-001\nstatus: ready\n---\n").unwrap();
    let mut child=Command::new(env!("CARGO_BIN_EXE_lmbrain-mcp")).current_dir(d.path()).stdin(Stdio::piped()).stdout(Stdio::piped()).spawn().unwrap();
